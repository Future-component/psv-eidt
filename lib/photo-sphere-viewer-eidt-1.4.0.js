...
(function(global, factory) {
  ...
  'use strict';
	...
	var removeEvent = function(elt, evt, f) {
  ...
  function PhotoSphereViewer(args) {
      ...
      createBuffer(panorama).then(function(path) {
		...
		var endEvent = function() {
			// Adding events
			removeEvent(window, 'resize', fitToContainer);

			if (user_interactions_allowed) {
				removeEvent(canvas_container, 'mousedown', onMouseDown);
				removeEvent(document, 'mousemove', onMouseMove);
				removeEvent(document, 'mouseup', onMouseUp);

				removeEvent(canvas_container, 'touchstart', onTouchStart);
				removeEvent(document, 'touchend', onMouseUp);
				removeEvent(document, 'touchmove', onTouchMove);

				if (scroll_to_zoom) {
					removeEvent(canvas_container, 'mousewheel', onMouseWheel);
					removeEvent(canvas_container, 'DOMMouseScroll', onMouseWheel);
				}
			}
		}
	...
	PhotoSphereViewer$1.Version = Version;
